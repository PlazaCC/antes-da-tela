...
const BARS = [
  60, 78, 64, 82, 72, 95, 88, 76, 90, 58, 84, 100, 72, 66, 92, 80, 70, 86, 64,
  78,
]
    ...
    <div className="flex flex-col gap-5">
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {CARDS.map((c) => (
          <div
            key={c.label}
            className="flex flex-col gap-1.5 rounded-[2px] border border-[rgb(37,37,37)] bg-[rgb(14,14,14)] p-[14px]"
          >
            <span className="font-mono text-[10px] uppercase tracking-[0.1em] text-[rgb(107,104,96)]">
              {c.label}
            </span>
            <Counter to={c.to} suffix={c.suffix} start={reveal} />
            <span
              className={`font-mono text-[10px] uppercase tracking-[0.1em] ${c.up ? 'text-[rgb(60,190,126)]' : 'text-[rgb(239,69,69)]'}`}
            >
              {c.delta}
            </span>
          </div>
      ...
      <div className="rounded-[2px] border border-[rgb(37,37,37)] bg-[rgb(14,14,14)] p-[18px_20px_20px]">
        <div className="mb-5 flex items-center justify-between">
          ...
          <div className="flex gap-3.5 font-mono text-[10px] uppercase tracking-[0.1em] text-[rgb(107,104,96)]">
            <span>
              <i className="mr-1.5 inline-block h-[10px] w-[10px] bg-[hsl(var(--color-brand-accent))] align-middle" />
              reações
            </span>
            <span>
              <i className="mr-1.5 inline-block h-[10px] w-[10px] bg-[rgb(107,104,96)] align-middle" />
              leituras
            </span>
          </div>
        </div>
        ...
        <div className="flex justify-between font-mono text-[10px] uppercase tracking-[0.1em] text-[rgb(107,104,96)]">
          {['cena 01', 'cena 12', 'cena 24', 'cena 36'].map((l) => (
            <span key={l}>{l}</span>
          ))}
        </div>
      </div>
